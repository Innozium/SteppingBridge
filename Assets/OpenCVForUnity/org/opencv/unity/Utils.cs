...
//						#endif
						...
						if (mat.type () == CvType.CV_8UC4) {
								...
								OpenCVForUnity_LowLevelTextureToMat (webCamTexture.GetNativeTexturePtr (), webCamTexture.width, webCamTexture.height, mat.nativeObj);	
								if (Utils.getGraphicsDeviceType () == 16 && Utils.getLowLevelTextureFormat (webCamTexture) == 80) {
     ...
     */
		public static int getGraphicsDeviceType ()
		{
			...
			return OpenCVForUnity_GetGraphicsDeviceType ();
     ...
     * <br>Direct3D 11 DXGI_FORMAT (Win WindowsStoreApp WindowsPhone)
     * <br>OpenGL internalFormat (Win OSX Linux)
     ...
     */
		...
		public static string getFilePath (string filename)
		{
			...
			using (AndroidJavaClass unityPlayerJClass = new AndroidJavaClass("com.unity3d.player.UnityPlayer"))
			using (AndroidJavaObject currentActivityJObject = unityPlayerJClass.GetStatic<AndroidJavaObject>("currentActivity")) 
			using (AndroidJavaClass openCVForUnityUnityPluginJClass = new AndroidJavaClass("com.enoxsoftware.opencvforunity.OpenCVForUnityPlugin")) {
				return openCVForUnityUnityPluginJClass.CallStatic<string> ("copyFileFromAssets", currentActivityJObject, filename, "opencvforunity");
			}
			#else
		...
		[DllImport("__Internal")]
		private static extern int OpenCVForUnity_GetGraphicsDeviceType ();
